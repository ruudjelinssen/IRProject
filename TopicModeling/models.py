...
def DTM(corpus, dictionary):
    ...
    if not os.path.exists(DTM_MODEL_FILE):
        model = gensim.models.wrappers.DtmModel('dtm-win64.exe', corpus,
                                                time_slices, num_topics=20,
                                                id2word=dictionary)
        model.save(DTM_MODEL_FILE)
    else:
        model = gensim.models.wrappers.DtmModel.load(DTM_MODEL_FILE)
        logging.info("Using Existing DTM model")
